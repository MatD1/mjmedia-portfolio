...
export async function generateMetadata({ params }: ProjectPageProps) {
  const project = await api.project.getById({ id: parseInt(params.id) });
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
                ...
                {project.images.length > 1 && (
                  <div className="grid grid-cols-2 gap-4">
                    {project.images.slice(1, 5).map((image, index) => (
                      <div key={index} className="aspect-video bg-[var(--bg-tertiary)] rounded border border-[var(--border-primary)] overflow-hidden">
                        <Image
                          src={image}
              ...
              <div className="space-y-6">
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  {project.liveUrl && (
                    <Button size="lg" className="flex items-center gap-2 flex-1">
                      <IoGlobe />
                      View Live Demo
                    </Button>
                  )}
                  {project.githubUrl && (
                    <Button variant="secondary" size="lg" className="flex items-center gap-2 flex-1">
                      <IoLogoGithub />
                      View Source
                    </Button>
                  )}
