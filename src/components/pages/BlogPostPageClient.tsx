...
type BlogPost = NonNullable<RouterOutputs["blog"]["getById"]>;

interface BlogPostPageClientProps {
  blog: BlogPost;
}

export default function BlogPostPageClient({ blog }: BlogPostPageClientProps) {
          ...
          <Card className="p-8">
            <div className="prose prose-invert max-w-none">
              <ReactMarkdown
                ...
                components={{
                  h1: ({ children }) => (
                    <h1 className="pixel-text text-2xl text-glow mb-6 mt-8 first:mt-0">
                      {children}
                    </h1>
                  ),
                  h2: ({ children }) => (
                    <h2 className="pixel-text text-xl text-glow mb-4 mt-6">
                      {children}
                    </h2>
                  ),
                  h3: ({ children }) => (
                    <h3 className="pixel-text text-lg text-glow mb-3 mt-5">
                      {children}
                    </h3>
                  ),
                  h4: ({ children }) => (
                    <h4 className="pixel-text text-base text-glow mb-2 mt-4">
                      {children}
                    </h4>
                  ),
                  p: ({ children }) => (
                  ...
                  img: ({ src, alt }) => {
                    ...
                    // Handle both absolute and relative URLs and possible Blob values
                    let srcUrl: string;
                    ...
                    if (typeof src === "string") {
                      srcUrl = src;
                      isRelativeUrl =
                        srcUrl.startsWith("/api/media-alt/") ||
                        srcUrl.startsWith("/api/media/");
                    } else if (src instanceof Blob) {
                      // Create an object URL for Blob sources
                    ...
                    return (
                      <div className="my-8">
                        <Image
                          ...
                          style={{ height: "auto" }}
                          unoptimized={!isRelativeUrl} // Disable optimization for external URLs
                          priority={false}
                          onLoadingComplete={() => {
                            // Revoke object URL if we created one for a Blob to avoid memory leaks
                            if (
                              typeof src !== "string" &&
                              src instanceof Blob
                            ) {
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-[var(--text-secondary)] mb-4">
                  ...
                  experiences that blend modern web technologies with retro
                  gaming aesthetics.
                </p>
